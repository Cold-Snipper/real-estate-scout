...
export function WebsiteScan() {
  ...
  useEffect(() => {
    if (prefillDone.current || !targets?.countries?.length) return;
    if (country !== "") return;
    prefillDone.current = true;
    setCountry("Luxembourg");
    const luSites = targets.sitesByCountry?.["Luxembourg"];
    if (luSites?.some((s) => s.id === "athome")) setSiteId("athome");
    else if (luSites?.length) setSiteId(luSites[0].id);
  }, [targets, country]);
