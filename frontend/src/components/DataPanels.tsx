              ...
              <tbody>
                {leads.slice(0, 15).map((l) => (
                  <tr key={l.id} className="border-b border-border/50 hover:bg-secondary/30">
                    ...
                    <td className="py-1.5 pr-3">{(l as Record<string, unknown>)['contact_phone'] as string || "—"}</td>
                    ...
                    <td className="py-1.5 font-mono text-[10px] truncate max-w-[200px]">
                      {l.listing_url ? (
                        <a href={l.listing_url} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline">
                          {(() => { try { return new URL(l.listing_url).hostname; } catch { return l.listing_url; } })()}
                        </a>
                      ) : "—"}
                    </td>
                  </tr>
